...
# first place. For example, the rotation of
...
#     lambda change_position = index + K % len(A)


def solution(A: list[int], K: int = 4) -> list[int]:
    """100% score"""
    if not A:
        return []
    for _ in range(K):
...
tests = [[3, 8, 9, 7, 6], [0, 0, 0], [1, 2, 3, 4]]
results = [[8, 9, 7, 6, 3], [0, 0, 0], [1, 2, 3, 4]]

for i, test in enumerate(tests):
    text = f"{test} -> {results[i]}"
    res = solution(test)
    print("[Pass]" if res == results[i] else "[Fail]", text)
